...
> 说明：截图和第二、三节的数值来自修复前版本，用于解释当时的抖动根因；当前参数和生命周期规则以第一节、`fix.md` 及 `协议.md` 为准。



## 一、方案落实情况：原 6 条基础修复与时序收敛已完成

| 方案                                        | 状态 | 代码                                                       |
| ------------------------------------------- | ---- | ---------------------------------------------------------- |
| 1 参考缓存 TTL / 内容硬时效拆分为 1000/450ms，future 100ms | 已改 | `rebuild_receiver.py`、`live_h265_hud.py` |
...
`REF 1/1 AGE 706ms` 里的 AGE 是 `now - reference.received_at`，从**收齐那一刻**算起，不是从拍摄那一刻；现在它只用于显示和缓存 TTL，
不再作为内容显示有效期。
...
所以真实内容年龄 ≈ 706 + 670 ≈ **1.4 秒**，test2 是 848 + 670 ≈ **1.5 秒**。旧版 `patch_refresh_ms=700` 实际上达不到安全时效；
当前发送端改为结合共享 token bucket、参考大小、FEC 和已观测 P95 传输时长的 deadline-driven 刷新，接收端超过 `450 ms` 必须回退 BASE。
...
4. **年龄闸门改用 `video_pts - reference.pts_ms`**，`now - received_at` 仅作为缓存 TTL/诊断 AGE；`<=450 ms` 才能显示，future 超过 `100 ms` 也拒绝。
...
- 完整 PATCH 通过 FEC、CRC、JPEG 和 mask 校验后立即 local-ready，不等待 ready STATE；late STATE 不得把本地新代次倒退。
- Real-ESRGAN 保持 `1 running + 1 latest pending`，提交前和完成后都复查内容年龄、future、generation、reference_generation 和 track。
- 每个 source sequence 只合成一次；异步 SR 结果只能进入后续 source frame，同一帧的所有 HOLD tick 复用同一组像素。
- sender 使用软提示 `350 ms`、硬时限 `450 ms`、guard `75 ms` 的带宽感知刷新，以参考采集时间和源帧量化机会调度，并记录 capture/encode/queue/send/完成延迟和 P50/P95。

## 八、goal0831 根因闭环与最终实测

`debug.txt` 的 389 帧基线中，`BASE-LANCZOS=178`、`ROI-ESRGAN=137`、`ROI-LANCZOS=74`，
参考代次间隔中位数约 `501 ms`，所以“给了 100 kbps 仍看不见人”并不是单纯的链路带宽不足。
发送端实际按包完成时间重置刷新保持，6 fps 离散机会把目标参考推迟到约 `500/667 ms`；接收端
又把正常移动框的中心变化当成非法几何，SR 虽然可能完成却被未来时序和同源帧不可变规则挡在显示门外。

当前版本的修复是：

- 以参考采集 PTS 作为刷新年龄，使用 `350 ms` cadence hint、`450 ms` hard deadline、`75 ms` guard 和约 `167 ms` 调度量化，慢发送按估计传输时延提前启动；
- 几何注册取消固定中心 25% 拒绝，保留无效几何、极端宽高/面积比、完全越界和内容相关度 fail-closed；
- SR 拆分计算门和渲染门，保持 `1 running + 1 latest pending`，future 参考可预计算/缓存但不得提前渲染；缓存按 `(generation, track_id, reference_generation)` 保持同源帧像素稳定。

Ubuntu `192.168.0.10` 交叉编译并部署到 `192.168.0.101` 后，同一 bus 人物视频运行 `180` 帧约 30 秒：
PC 收到 164 个 source frame，`ROI-LANCZOS=88`、`ROI-ESRGAN=76`、`BASE-LANCZOS=0`，
`REFREADY=164/164`、`USED=164/164`，所有内容/状态/几何/尺度/匹配 drop 均为 0，视频 `lost/reorder/decode_errors=0/0/0`。
刷新间隔收敛到 `p50=334 ms`、`p95=335 ms`、最大 `337 ms`，SR 延迟为 `p50=90 ms`、`p95=102 ms`。
最终包 `runs\goal0831_rk3588_final.tar.gz` 的 SHA-256 为
`638252aa3269e3d404811fa37cc0d145bbc7d0a1e123206847b6cd645c3e6f77`；完整日志见
`runs\goal0831_sender_final.log` 和 `runs\goal0831_receiver_final.log`。
